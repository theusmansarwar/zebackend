...
const createTeamMember = async (req, res) => {
  try {
    ...
    if (!role)
      missingFields.push({ name: "role", message: "Role is required" });
    if (!category)
...
const updateTeamMember = async (req, res) => {
  try {
    const { id } = req.params;
    let { name, role, description, category, socialLinks, published } =
      req.body;
    const image = req.file ? `/uploads/${req.file.filename}` : null;
    ...
    if (role) {
      ...
      role = { _id: roleExists._id, name: roleExists.name }; // Store full category object
    }
...
const deleteAllTeamMembers = async (req, res) => {
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return res
        .status(400)
        .json({ message: "Invalid request. Provide Team IDs." });
    }
